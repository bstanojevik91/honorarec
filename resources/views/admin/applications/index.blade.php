@extends('admin.layouts.app', [
    'pageTitle' => 'Апликации',
    'pageDescription' => 'Преглед и филтрирање на сите пристигнати апликации.',
])

@section('content')
    <div class="mb-6 rounded-[1.6rem] bg-white p-6 shadow-[0_20px_45px_-34px_rgba(15,23,42,0.2)]">
        <form method="GET" action="{{ route('admin.applications.index') }}" class="grid gap-4 md:grid-cols-[1fr_auto_auto]">
            <select name="job" class="block w-full rounded-2xl border-slate-200 px-4 py-3 text-sm focus:border-emerald-500 focus:ring-emerald-100">
                <option value="">Сите огласи</option>
                @foreach ($jobs as $job)
            ...
            <a href="{{ route('admin.applications.index') }}" class="inline-flex items-center justify-center rounded-2xl border border-slate-200 px-6 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                Исчисти
            </a>
        </form>
    </div>
                    ...
                    @forelse ($applications as $application)
                        <tr>
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('admin.applications.show', $application) }}" class="font-semibold text-emerald-700 transition hover:text-emerald-600">Отвори</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-slate-500">Нема апликации за прикажување.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
