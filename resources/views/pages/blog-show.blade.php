    ...
    <div class="relative isolate overflow-hidden bg-slate-950">
        ...
        <section class="relative mx-auto max-w-7xl px-4 pb-14 pt-28 sm:px-6 sm:pb-18 sm:pt-32 lg:px-8 lg:pb-24 lg:pt-44">
            <div class="max-w-5xl text-white">
                ...
                <h1 class="mt-5 max-w-5xl text-[2.15rem] font-extrabold tracking-tight sm:text-5xl lg:text-[4rem] lg:leading-[1.02]">{{ $post['title'] }}</h1>
                <p class="mt-5 max-w-3xl text-sm leading-7 text-slate-200 sm:mt-8 sm:text-lg sm:leading-9">{{ $post['intro'] }}</p>
            </div>
        </section>
    </div>

    <main class="bg-stone-50">
        <section class="mx-auto max-w-7xl px-4 pb-12 sm:px-6 lg:px-8 lg:pb-18">
            <div class="grid gap-8 pt-[15px] lg:grid-cols-[minmax(0,1.55fr)_minmax(18rem,0.7fr)] lg:gap-12">
                <article class="rounded-[1.45rem] border border-slate-200 bg-white p-5 shadow-[0_20px_45px_-34px_rgba(15,23,42,0.16)] sm:rounded-[1.8rem] sm:p-8 lg:p-12">
                    <div class="border-b border-slate-100 pb-8">
                        ...
                        <p class="mt-4 max-w-3xl text-sm leading-7 text-slate-600 sm:mt-5 sm:text-[1.02rem] sm:leading-8">{{ $post['excerpt'] }}</p>
                    </div>

                    @if (!empty($post['content']))
                        <div class="mt-8 max-w-3xl space-y-10 sm:mt-12 sm:space-y-14">
                            <div class="rounded-[1.2rem] border border-emerald-100 bg-emerald-50/70 px-5 py-4 sm:rounded-[1.4rem] sm:px-6 sm:py-5">
                            ...
                            <div class="blog-content space-y-5 text-[0.97rem] leading-8 text-slate-600 sm:space-y-6 sm:text-[1.02rem] sm:leading-9 [&_h2]:mt-10 [&_h2]:text-[1.55rem] [&_h2]:font-extrabold [&_h2]:tracking-tight [&_h2]:text-slate-900 sm:[&_h2]:mt-12 sm:[&_h2]:text-[2rem] [&_h3]:mt-8 [&_h3]:text-lg [&_h3]:font-bold [&_h3]:tracking-tight [&_h3]:text-slate-900 sm:[&_h3]:mt-10 sm:[&_h3]:text-xl [&_p]:mt-5 [&_p]:leading-8 sm:[&_p]:mt-6 sm:[&_p]:leading-9 [&_ul]:mt-6 [&_ul]:space-y-3 [&_ul]:pl-5 sm:[&_ul]:mt-7 sm:[&_ul]:space-y-4 sm:[&_ul]:pl-6 [&_li]:list-disc [&_li]:pl-1 [&_a]:font-semibold [&_a]:text-emerald-700 [&_a]:underline [&_strong]:font-semibold [&_strong]:text-slate-900">
                                {!! $post['content'] !!}
                            </div>
                    ...
                    @else
                        <div class="mt-8 max-w-3xl space-y-10 sm:mt-12 sm:space-y-14">
                            @foreach ($post['sections'] as $index => $section)
                                <section class="{{ $index > 0 ? 'border-t border-slate-100 pt-10 sm:pt-12' : '' }}">
                                    <h2 class="text-[1.55rem] font-extrabold tracking-tight text-slate-900 sm:text-[2.15rem]">{{ $section['heading'] }}</h2>
                                    <p class="mt-5 text-[0.97rem] leading-8 text-slate-600 sm:mt-6 sm:text-[1.02rem] sm:leading-9">{{ $section['content'] }}</p>

                                    @if (!empty($section['subheading']))
                                        <h3 class="mt-8 text-lg font-bold tracking-tight text-slate-900 sm:mt-10 sm:text-[1.45rem]">{{ $section['subheading'] }}</h3>
                                    @endif

                                    @if (!empty($section['points']))
                                        <ul class="mt-6 space-y-3 pl-5 text-[0.97rem] leading-8 text-slate-600 sm:mt-7 sm:space-y-4 sm:pl-6 sm:text-[1.02rem] sm:leading-9">
                                    ...
                                    @if ($index === 0)
                                        <blockquote class="mt-8 rounded-[1.2rem] border border-emerald-100 bg-emerald-50/70 px-5 py-4 sm:mt-10 sm:rounded-[1.4rem] sm:px-6 sm:py-5">
                                            <p class="text-base font-semibold leading-8 text-emerald-900">
                                                „Најдобро поминуваат кандидатите што аплицираат брзо, јасно и со точни контакт податоци.“
                                            </p>
                                        </blockquote>
                                    ...
                                    @if (!empty($section['points']))
                                        <div class="mt-8 rounded-[1.2rem] border border-slate-200 bg-slate-50 px-5 py-4 sm:mt-10 sm:rounded-[1.4rem] sm:px-6 sm:py-5">
                                            <p class="text-sm font-semibold uppercase tracking-[0.18em] text-slate-500">Корисен совет</p>
                ...
                <aside class="space-y-4 sm:space-y-5">
                    <div class="rounded-[1.35rem] border border-slate-200 bg-white p-4 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.14)] sm:rounded-[1.6rem] sm:p-5">
                    ...
                    <div class="rounded-[1.35rem] border border-slate-200 bg-white p-4 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.14)] sm:rounded-[1.6rem] sm:p-5">
                        ...
                        <div class="mt-4 flex flex-wrap gap-2.5 sm:mt-5">
                            @foreach ($categories as $category)
                                <span class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3.5 py-2 text-sm font-semibold text-slate-700 transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700 sm:px-4">
                                    {{ $category['name'] }}
